...
export default function Home() {
  return (
    <div className="max-w-3xl mx-auto p-6">
      <h1 className="text-2xl font-medium mb-6">피드</h1>
...
async function Feed() {
  ...
  // 공개 이력서 (RLS가 is_public = true 읽기 허용)
  const { data: resumes } = await supabase
    .from("resumes")
    .select(
      "id, title, description, user_id, experience_years, like_count, save_count, view_count",
    )
    .eq("is_public", true)
  ...
  if (!resumes?.length) {
    return (
      <div className="border border-dashed rounded-lg p-12 text-center text-gray-500">
        <p className="mb-2">아직 공개된 이력서가 없어요</p>
        <p className="text-sm">첫 번째로 이력서를 공개해보세요</p>
      </div>
    );
  }
